...
```bash
# 查看你当前的 Shell / Check your current shell
echo $SHELL

# 查看系统有哪些 Shell / List available shells
cat /etc/shells
```
...
| options | 以 `-` 或 `--` 开头，控制行为 starts with `-` or `--` | `-l`, `--help` |
| arguments | 命令操作的对象 the target | `/home`, `file.txt` |

**关键原则：一切皆文件。** 键盘、屏幕、磁盘、进程、网络连接在 Linux 中都以文件形式呈现。
...
ls /nonexistent
echo $?     # 打印上一条命令的退出码 / prints last command's exit code
```
...
> :warning: `rm` 没有回收站！删了就没了。No recycle bin — deletions are permanent.
...
man ls          # q 退出 / q to quit, / 搜索 / to search, n 下一个 / next
...
command > out.txt 2>&1      # stdout 和 stderr 都写到一个文件 / both to same file

# 管道：左边命令的输出 → 右边命令的输入
# Pipe: left command's output → right command's input
cat /var/log/syslog | grep error | head -n 10
ls -l | sort -k5 -n          # 按第 5 列（文件大小）数字排序 / sort by file size
```

> **关键思维：** Linux 命令遵循 "Do One Thing Well" 哲学。每个命令只做一件事，用管道组合起来完成复杂任务。
>
> **Key mindset:** Linux commands follow the "Do One Thing Well" philosophy. Each does one thing; combine them with pipes for complex tasks.
...
Run `bash check.sh` in the `exercises/` directory to verify your work.

1. 在 `$HOME/linux-lab/temp/` 下创建目录 `chapter1/test/`
2. 用一条命令创建文件 `a.txt`、`b.txt`、`c.txt`
3. 将 `$HOME/linux-lab/files/fruits.txt` 中所有包含 "apple" 的行输出到一个新文件
